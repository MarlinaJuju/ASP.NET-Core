...
namespace BookReviewAPI.Controllers
{
    ...
    public class BookController : Controller
    {
        ...
        public BookController(IBookReviewRepository repo)
        {
            _repository = repo;
        }
        ...
        {
            return Ok(Mapper.Map<List<BookDTO>>(_repository.GetBooks()));
        }

        [HttpGet("book/publisher/{publisherId}")]
        public IActionResult GetBooksByPublisherId(int publisherId)
        {
            return Ok(Mapper.Map<List<BookDTO>>(_repository.GetBooksByPublisherId(publisherId)));
        }

        [HttpGet("book/author/{authorId}")]
        public IActionResult GetBooksByAuthorId(int authorId)
        {
            return Ok(Mapper.Map<List<BookDTO>>(_repository.GetBooksByAuthorId(authorId)));
        }
        ...
        {
            return Ok(Mapper.Map<List<BookDTO>>(_repository.GetBooksByAuthorIdAndPublisherId(authorId, publisherId)));
        }

        [HttpGet("book/{id}")]
        public IActionResult GetBook(int id)
        {
            var book = _repository.GetBook(id, true);
            if (book == null)
            {
                return NotFound();
            }
        ...
        public IActionResult EditBook(int id, [FromBody]BookInsertDTO bookInsertDTO)
        {
            ...
            var book = _repository.GetBook(id,false);
            if (book == null)
            {
                return NotFound();
            }
